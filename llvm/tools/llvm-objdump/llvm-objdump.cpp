...
static void printMarkupSpans(StringRef Text, ArrayRef<MarkupSpan> Spans,
  ...
  for (const MarkupSpan &Span : Spans) {
    ...
    if (Span.InnerSpans.empty()) {
      ...
      withHighlightColor(Color) << Text.substr(InnerPos, Span.InnerLength);
    } else {
      NextPos = InnerPos;
      printMarkupSpans(Text, Span.InnerSpans, NextPos, InstOffset);
      StringRef AfterText =
...
static void printMarkedUpInst(StringRef Text,
  ...
  size_t NextPos = 0;
  printMarkupSpans(Disasm, Spans, NextPos, InstOffset);
    ...
    for (unsigned SI = 0, SE = Symbols.size(); SI != SE; ++SI) {
      ...
      while (Index < End) {
        ...
        PIP.printInst(*IP, Disassembled ? &Inst : nullptr,
                      ...
                      InstTextStream, "", *STI, &SP, Obj->getFileName(), InstructionCol, &Rels);
