...
class Section {
private:
  std::vector<uint8_t> OwnedContentData;

public:
  std::string Sectname;
  std::string Segname;
  // CannonicalName is a string formatted as “<Segname>,<Sectname>".
  ...
  Section(StringRef SegName, StringRef SectName)
      : Sectname(SectName), Segname(SegName),
        CannonicalName((Twine(SegName) + Twine(',') + SectName).str()) {}
  ...
  void setOwnedContentData(ArrayRef<uint8_t> Data) {
    OwnedContentData.assign(Data.begin(), Data.end());
    Content = StringRef(reinterpret_cast<const char *>(OwnedContentData.data()),
